...
import { getApplicationSubmitPath, getNextApplicationStep, hasCompletedCv, hasCompletedPersonalInfo, hasCompletedProfile, setPendingApplication } from "../utils/applicationFlow";
...
const JobDetails = () => {
  ...
  const handleApplyAction = () => {
    ...
    if (!isAuthenticated) {
      ...
      const complete = hasCompletedCv() && hasCompletedPersonalInfo() && hasCompletedProfile();
          ...
          <div>
            <button
              ...
              className={`w-full md:w-auto px-8 py-3.5 font-medium rounded-lg shadow-sm transition-all flex items-center justify-center gap-2 ${
                "bg-[#0871D1] text-white hover:bg-[#075EAE]"
              }`}
            >
              {!isAuthenticated ? (
                <>
