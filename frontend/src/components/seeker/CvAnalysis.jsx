...
const AiCvAnalysis = () => {
  ...
  const continueToProfile = (event) => {
    ...
    setSaving(true);
    try {
      const pendingData = { ...analysis, skills };
  ...
  const updateField = (field, value) => setAnalysis((current) => ({ ...current, [field]: value, ...(field === 'fullName' ? { full_name: value } : {}), ...(field === 'headline' ? { professional_title: value } : {}) }));
